...
export function PublishForm({ onClose }: Props) {
  ...
  // 默认 form 用 platform_copy 自动填; 没文案就空, 用户自己填
  const [forms, setForms] = useState<Record<Platform, FormState>>(() => ({
    xhs: {
      title: copyDefaults?.xiaohongshu?.title || '',
      description: copyDefaults?.xiaohongshu?.body || '',
      tags: (copyDefaults?.xiaohongshu?.tags || []).join(', '),
    },
    douyin: {
      title: copyDefaults?.douyin?.title || '',
      description: copyDefaults?.douyin?.description || '',
      tags: (copyDefaults?.douyin?.tags || []).join(', '),
    },
  }))
