...
const api = {
    ...
    detectEdge: async () => electron_1.ipcRenderer.invoke('detect-edge'),
    /** 调本地 Edge 发布到平台. 同步阻塞: 主进程跑 Playwright, renderer 转圈等 */
    publish: async (req) => electron_1.ipcRenderer.invoke('publish', req),
    // TODO Phase 4-3 加:
    // recordStart / recordStop: ffmpeg 抓屏
    // checkUpdate / installUpdate: electron-updater
};
electron_1.contextBridge.exposeInMainWorld('monoiDesktop', api);
