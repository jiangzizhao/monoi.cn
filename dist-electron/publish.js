...
exports.detectEdgePath = detectEdgePath;
exports.publishToXhs = publishToXhs;
...
function getEdgeProfileDir() {
    return path.join(electron_1.app.getPath('userData'), 'edge-profile');
}
...
function detectEdgePath() {
    ...
    const candidates = [];
    if (platform === 'win32') {
        candidates.push('C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe', 'C:\\Program Files\\Microsoft\\Edge\\Application\\msedge.exe');
    }
...
async function launchEdgePersistent(headless = false) {
    ...
    return await playwright_core_1.chromium.launchPersistentContext(profileDir, launchOpts);
}
// ============== 拟人化工具 ==============
...
async function mouseJitter(page, n = 2) {
...
async function publishToXhs(req) {
    const detailMsgs = [];
    const step = (msg) => { console.log(`[publish_xhs] ${msg}`); detailMsgs.push(msg); };
